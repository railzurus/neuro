...
export async function renderMantra(
  ...
  // Синтез бросает VoiceError при сбое — и это правильно: раньше здесь
  // собиралась запись из одной музыки, без голоса. Человек скачивал негодный
  // файл, он попадал в кэш браузера и возвращался оттуда при каждой следующей
  // попытке, а пересборка при этом уже была списана.
  const voiceBuffer = await synthesizeVoice(finalText, voice, speed, orderToken)
  const blob = await renderMix(voiceBuffer.duration + 8, voiceBuffer)
