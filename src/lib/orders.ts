...
/**
 * Прежняя цена, ₽ — показывается зачёркнутой рядом с текущей.
 * Это реально действовавшая цена, а не «маркетинговая»: по 299 ₽ мы начали
 * продавать 31 августа 2026 г., до этого сервис стоил 499 ₽. Требование
 * ч. 3 ст. 5 ФЗ «О рекламе» — сравнивать можно только с настоящей ценой.
 * Если старая цена перестанет быть правдой, эту константу нужно убрать,
 * а не подгонять.
 */
export const PRICE_RUB_OLD = 499

/** Скидка в процентах — для подписи «−40%». */
export const PRICE_DISCOUNT_PCT = Math.round((1 - PRICE_RUB / PRICE_RUB_OLD) * 100)
